...
$data merge storage 3d_triggers {$(node):{id:$(node),x:0d,y:0d,z:0d,dx:$(dx),dy:$(dy),dz:$(dz),command:"$(command)",single:$(single),selector:"$(selector)",offset_x:0.0,offset_y:0.0,offset_z:0.0,color:$(color)}}
...
execute store result score 3d_triggers.temp1 3d_triggers run data get entity @s Pos[0] 100
execute store result score 3d_triggers.temp2 3d_triggers run data get entity @s Pos[1] 100
execute store result score 3d_triggers.temp3 3d_triggers run data get entity @s Pos[2] 100

$execute store result storage 3d_triggers $(node).x double 0.01 run scoreboard players get 3d_triggers.temp1 3d_triggers
$execute store result storage 3d_triggers $(node).y double 0.01 run scoreboard players get 3d_triggers.temp2 3d_triggers
$execute store result storage 3d_triggers $(node).z double 0.01 run scoreboard players get 3d_triggers.temp3 3d_triggers

scoreboard players reset 3d_triggers.temp1 3d_triggers
scoreboard players reset 3d_triggers.temp2 3d_triggers
scoreboard players reset 3d_triggers.temp3 3d_triggers
...
# Dev mode debug
$execute if score KFrame.dev_mode KFrame matches 1 run tellraw @a {"text":"[K-Frame]","color":"aqua","bold":true,"extra":[{"text":" [3D Triggers]: ","color":"green","bold":false},{"text":"Created node at player: ","color":"gold","bold":false},{"text":"$(node) ","color":"yellow","bold":false}]}
